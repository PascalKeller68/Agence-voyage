...
class WTE_Cart {
	 ...
	 * @since   2.2.6
	 */
	public function get_cart_item_id( $trip_id, $pricing_id = '', $start_date = '', $time = '' ) {
		$suffix       = ( new DateTime( $start_date . ' ' . $time ) )->format( 'dmYHi' );
		$cart_item_id = "cart_{$trip_id}";

		foreach ( array( 'pricing_id', 'start_date', 'time' ) as $param ) {
			if ( ! empty( $$param ) ) {
