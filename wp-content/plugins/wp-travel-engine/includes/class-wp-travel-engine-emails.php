			...
			foreach ( $order_trips as $trip ) :
				...
				<table width="100%" cellpadding="0" cellspacing="0">
					...
					<tr>
						...
						<td class="alignright"><?php echo date_i18n( 'j M, Y', strtotime( $trip->datetime ) ); ?></td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td class="alignright">
							<table width="100%" cellpadding="0" cellspacing="0">
								<?php
								$sum = 0;
								foreach ( $trip->pax as $label => $tcount ) {
									if ( +$tcount < 1 ) {
										continue;
									}
									$pax_cost = +$trip->pax_cost[ $label ] / +$tcount;
									$sum     += +$trip->pax_cost[ $label ];
									echo '<tr>';
									echo "<td class=\"alignright\">{$label}</td>";
									echo "<td>{$tcount} X " . wte_get_formated_price( $pax_cost, $currency, '', ! 0 ) . ' = ' . wte_get_formated_price( $trip->pax_cost[ $label ], $currency, '', ! 0 ) . '</td>';
									echo '</tr>';
								}
								?>
								<tr>
